...
type Config struct {
	...
	// DisableChallengeValidation marks ACME client challenges (http-01,
	// dns-01, tls-alpn-01) as valid without outbound validation. Intended
	// for ACME proxy deployments where domain control is proven toward the
	// upstream CA and the proxy cannot resolve/reach every client domain.
	DisableChallengeValidation bool `json:"disableChallengeValidation,omitempty"`
...
// LoadedFilepath is the path of the last ca.json successfully loaded by
// LoadConfiguration. Plugins in the enclosing binary use it to re-read
// top-level configuration blocks that do not belong under "authority".
var LoadedFilepath string

// StripJSONComments removes '#' comments (from '#' to end of line) that
// appear outside JSON string literals, so plain JSON parsers accept
// comment-annotated configuration files.
...
// configuration struct. '#' comments outside string literals are allowed.
