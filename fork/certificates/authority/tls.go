...
// tlsKeyMutex serializes load-or-create of the persistent CA TLS key.
var tlsKeyMutex sync.Mutex

// tlsKeyPath returns the path of the file backing the CA HTTPS server private
// key. It defaults to tls_key.pem next to the ca.json config file and can be
// overridden with the top-level "tlsKeyFile" property in ca.json.
func (a *Authority) tlsKeyPath() string {
	if a.config.TLSKeyFile != "" {
		return a.config.TLSKeyFile
	}
	if f := a.config.Filepath(); f != "" {
		return filepath.Join(filepath.Dir(f), "tls_key.pem")
	}
	return ""
}

// loadOrCreateTLSKey returns the CA HTTPS server private key, persisting it
// to disk so that restarts (and in-process renewals) reuse the same key. A
// stable key allows the cached upstream certificate to be reused instead of
// forcing a new ACME order on every boot.
func (a *Authority) loadOrCreateTLSKey() (crypto.Signer, error) {
	path := a.tlsKeyPath()

	// Without a config file there is nowhere to persist the key; fall back
	// to an ephemeral key (previous behavior).
	if path == "" {
	...
	tlsKeyMutex.Lock()
	defer tlsKeyMutex.Unlock()

	if _, err := os.Stat(path); err == nil {
		priv, err := pemutil.Read(path)
		if err != nil {
			return nil, errors.Wrapf(err, "error reading TLS key %s", path)
		}
		signer, ok := priv.(crypto.Signer)
		if !ok {
			return nil, errors.Errorf("TLS key %s is not a crypto.Signer", path)
		}
		return signer, nil
	} else if !os.IsNotExist(err) {
		return nil, errors.Wrapf(err, "error checking TLS key %s", path)
	}
	...
	block, err := pemutil.Serialize(priv)
	if err != nil {
		return nil, errors.Wrap(err, "error serializing TLS key")
	}
	if err := os.WriteFile(path, pem.EncodeToMemory(block), 0600); err != nil {
		return nil, errors.Wrapf(err, "error writing TLS key %s", path)
	}
	return signer, nil
}
...
func (a *Authority) GetTLSCertificate() (*tls.Certificate, error) {
	...
	// Load or create the persistent TLS key so restarts reuse the cached
	// upstream certificate instead of requesting a new one per boot.
	signer, err := a.loadOrCreateTLSKey()
	if err != nil {
		return fatal(err)
	}
	...
	keyPEM, err := pemutil.Serialize(signer)
	if err != nil {
		return fatal(err)
	}
