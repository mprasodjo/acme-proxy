...
// identifiersDetail renders order identifiers as a compact comma-joined list
// for the request-meta hook.
func identifiersDetail(ids []acme.Identifier) string {
...
func NewOrder(w http.ResponseWriter, r *http.Request) {
	...
	now := clock.Now()
	if RequestMetaHook != nil {
		RequestMetaHook(r, "new-order", identifiersDetail(nor.Identifiers), nil, "")
	}
	// New order.
	o := &acme.Order{
		AccountID:        acc.ID,
...
func FinalizeOrder(w http.ResponseWriter, r *http.Request) {
	...
	if RequestMetaHook != nil {
		RequestMetaHook(r, "finalize", o.ID, fr.csr, "")
	}
