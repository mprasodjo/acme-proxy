...
var (
	// SkipChallengeValidation marks pending RFC 8555 challenges (http-01,
	// dns-01, tls-alpn-01) as valid without performing any outbound
	// validation. This is used by ACME proxy deployments where domain
	// control is proven toward the upstream CA and the proxy must be able
	// to resolve/reach every requesting client. WARNING: with this enabled
	// any caller able to reach the ACME provisioner can obtain certificates
	// for any name the upstream CA will issue — restrict network access
	// accordingly.
...
func (ch *Challenge) Validate(ctx context.Context, db DB, jwk *jose.JSONWebKey, payload []byte) error {
	...
	// Optionally skip outbound validation for the standard challenge types:
	// domain control is delegated to the upstream CA (proxy deployments).
